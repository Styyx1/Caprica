...
template <>
inline void CapricaBinaryWriter::write(identifier_ref val) {
  boundWrite<uint16_t>(val.size());
  if (val.size())
    append(val.data(), val.size());
}
...
template <>
inline void CapricaBinaryWriter::write(int8_t val) {
  strm.make<int8_t>(val);
}

template <>
inline void CapricaBinaryWriter::write(uint8_t val) {
